...
def search():
    ...
    try:
        ...
        # Return with empty data instead of redirecting so user can see the search form
        flash('Feil ved søk etter innsidehandel data. Viser demo data.', 'warning')
        
        # Return template with empty data but search params preserved
        return render_template('insider_trading/index.html',
                             insider_data=[],
                             popular_stocks=DataService.get_popular_stocks() or [],
                             selected_symbol=symbol,
