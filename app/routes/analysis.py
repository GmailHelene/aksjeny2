...
def short_analysis():
    ...
    try:
        ...
        if ticker:
            ...
            if DataService:
                data = DataService.get_short_analysis(ticker)
            else:
                data = None
                
            if not data:
                # Generate demo data if no real data available
                data = generate_demo_short_data(ticker)
        else:
            data = None
        ...
        current_app.logger.error(f"Short analysis error: {str(e)}")
        return render_template('analysis/short-analysis.html', 
                             ticker=ticker,
                             stock_data=None,
                             error=True)

def generate_demo_short_data(ticker):
    """Generate demo data for short analysis"""
    return {
        'ticker': ticker,
