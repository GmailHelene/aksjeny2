...
def access_required(f):
    """Decorator to check if user has access to the resource - STRICT ACCESS CONTROL"""
    @wraps(f)
    ...
    def decorated_function(*args, **kwargs):
        ...
        if current_user.is_authenticated:
            ...
            # For authenticated users WITHOUT subscriptions - redirect to demo
            from flask import flash, redirect, url_for
            flash('Du trenger et aktivt abonnement for å få tilgang til denne funksjonen. Prøv demo-versjonen.', 'info')
...
def check_access_and_redirect():
    ...
    # Public endpoints that should always be accessible (general pages, pricing, auth, etc.)
    public_endpoints = {
        'main.demo', 'main.login', 'main.register', 'main.logout',
        'main.about', 'main.contact', 'main.contact_submit', 'main.terms', 'main.privacy', 'main.privacy_policy',
        'main.help', 'main.features', 'main.forgot_password', 'main.reset_password',
        'main.offline', 'main.offline_html', 'main.service_worker', 'main.manifest', 'main.version',
        'static', 'favicon', 'health.health_check', 'main.landing',
        'pricing.index', 'pricing.pricing_page', 'pricing.subscription', 'pricing.manage_subscription',
        'pricing.stripe_webhook', 'pricing.cancel_subscription', 'pricing.payment_success', 'pricing.payment_cancel'
    }
