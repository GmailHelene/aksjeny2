...
class DataService:
    ...
    def get_comparative_data(symbols, period='6mo', interval='1d'):
        ...
        for symbol in symbols:
            ...
            # If historical data failed, create synthetic historical data from current real price
    ...
    def get_short_analysis(ticker):
        """Get short selling analysis for a stock"""
        try:
            # Get base stock info first
            stock_info = DataService.get_stock_info(ticker)
            if not stock_info:
                raise ValueError(f"Could not get base stock info for {ticker}")
                
            # Initialize yfinance for detailed data
            import yfinance as yf
            stock = yf.Ticker(ticker)
            
            # Get short data
            try:
                short_data = stock.get_info()
            except Exception as e:
                logger.warning(f"Failed to get short data for {ticker}: {e}")
                short_data = {}
            
            # Calculate short interest and other metrics
            short_interest = short_data.get('sharesShort', 0)
            float_shorted = (short_data.get('sharesShort', 0) / short_data.get('floatShares', 1)) * 100 if 'floatShares' in short_data else None
            days_to_cover = short_data.get('shortRatio', 0)
            
            # Get recent insider transactions
            try:
                insider_trades = stock.get_institutional_holders()
                insider_selling = len([x for x in insider_trades if x['Transaction'] == 'Sale']) if insider_trades is not None else 0
            except:
                insider_selling = 0
            ...
            try:
                ...
                recent_recs = recommendations.tail(30) if recommendations is not None else pd.DataFrame()
                analyst_downgrades = len(recent_recs[recent_recs['To Grade'] < recent_recs['From Grade']]) if not recent_recs.empty else 0
            except:
                analyst_downgrades = 0
                
            # Calculate short score (0-100)
            factors = []
            if float_shorted is not None:
                factors.append(min(float_shorted / 20 * 100, 100))  # Max score at 20% short
            if days_to_cover:
                factors.append(min(days_to_cover / 10 * 100, 100))  # Max score at 10 days
            if insider_selling:
                factors.append(min(insider_selling / 5 * 100, 100))  # Max score at 5 sales
            if analyst_downgrades:
                factors.append(min(analyst_downgrades / 3 * 100, 100))  # Max score at 3 downgrades
                
            short_score = sum(factors) / len(factors) if factors else 50
            ...
            return {
                ...
                'short_ratio': days_to_cover,  # Same as days to cover
    ...
    def get_stock_info(ticker):
        ...
        try:
            ...
            # LAST RESORT: Enhanced fallback data
            ...
            # Cache fallback data for longer to reduce future attempts
            if get_cache_service:
    ...
    def get_stock_data(ticker, period='1mo', interval='1d', fallback_to_cache=True):
        ...
        # If no real data available, use enhanced fallback as last resort
                ...
                try:
                    ...
                    if stock_info and stock_info.get('last_price', 0) > 0:
                        oslo_stocks[ticker] = {
                            ...
                            'change': stock_info.get('change', 0),
                            'change_percent': stock_info.get('change_percent', 0),
                            'open': stock_info.get('open', stock_info['last_price'] * 0.995),
    ...
    def get_insider_trading_data(symbol=None):
        """Get insider trading data with realistic fallback patterns based on symbol"""
        import random
        from datetime import datetime, timedelta
        
        # Create a base list of common transactions
        base_transactions = [
            {
                'company': 'Equinor ASA',
                'symbol': 'EQNR.OL',
        ...
        # If no symbol specified, return general insider trading data
        if not symbol:
            return base_transactions
        
        # Generate deterministic but varied data based on the symbol
        random.seed(hash(symbol))
        
        # Create company name from symbol if needed
        company_name = symbol.replace('.OL', '')
        ...
        elif company_name == 'YAR':
            company_name = 'Yara International ASA'
        elif company_name == 'NHY':
            company_name = 'Norsk Hydro ASA'
        else:
            # For other symbols, create a company name
            company_name = ' '.join([word.capitalize() for word in company_name.split('_')])
            if '.OL' in symbol:
                company_name += ' ASA'
        
        # Generate executives based on the company
        if '.OL' in symbol:
            # Norwegian executives
            executives = [
                {'name': 'Lars Hansen', 'position': 'CEO'},
                {'name': 'Ingrid Olsen', 'position': 'CFO'},
                {'name': 'Erik Johansen', 'position': 'Styreleder'},
                {'name': 'Kari Nilsen', 'position': 'Styremedlem'},
                {'name': 'Morten Andersen', 'position': 'Direktør'},
            ]
            currency = 'NOK'
            price_base = random.uniform(50, 300)
        else:
            # International executives
            executives = [
                {'name': 'John Smith', 'position': 'CEO'},
                {'name': 'Sarah Johnson', 'position': 'CFO'},
                {'name': 'Michael Brown', 'position': 'Chairman'},
                {'name': 'Jennifer Davis', 'position': 'Board Member'},
                {'name': 'Robert Wilson', 'position': 'Director'},
            ]
            currency = 'USD'
            price_base = random.uniform(100, 500)
        ...
        for i in range(num_transactions):
            # Pick an executive
            exec_index = random.randint(0, len(executives) - 1)
            executive = executives[exec_index]
            ...
            price = price_base * (1 + price_variation)
            
            # Shares - higher positions tend to have more shares
            position_factor = 1.0 if exec_index <= 1 else 0.5
            shares = int(random.uniform(1000, 20000) * position_factor)
            
            # Transaction type - more buys than sells is a positive signal
